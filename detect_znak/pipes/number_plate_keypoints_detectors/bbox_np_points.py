...
from detect_znak.tools.mcm import (modelhub, get_mode_torch)
from detect_znak.tools.image_processing import (distance,
                                                 get_cv_zone_rgb,
                                                 crop_image,
...
#                    device: str = "cuda",
                   ...
                   is_refine: bool = True,
                   trained_model: str = 0,
                   refiner_model: str = 0) -> None:
        """
        TODO: describe method
        """
        current_dir = os.path.dirname(os.path.abspath(__file__))
        craft_dir = os.path.join(current_dir, "../../../data/models/np_points_craft/craft_mlt/craft.onnx")
        refine_dir = os.path.join(current_dir, "../../../data/models/np_points_craft/craft_refiner/refiner.onnx")
        is_cuda = device == "cuda"
        self.is_cuda = is_cuda
        
        
        self.net = onnxruntime.InferenceSession(craft_dir,
                                                providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        # LinkRefiner
        self.refine_net = None
        if is_refine:
            
            self.refine_net = onnxruntime.InferenceSession(refine_dir,
                                                           providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
            self.is_poly = True
...
#         if self.is_cuda:
#             x = x.cuda()
        ort_inputs = {self.net.get_inputs()[0].name: x}
        y, feature = self.net.run(None, ort_inputs)
        refiner_inputs = {self.refine_net.get_inputs()[0].name: y, self.refine_net.get_inputs()[1].name: feature}
#         print(x.shape, y.shape, feature.shape)
        y_refiner = self.refine_net.run(None, refiner_inputs)
        y_refiner = y_refiner[0]
        # make score and link map
        score_text = y[0, :, :, 0]
        score_link = np.array(y_refiner)[0, :, :, 0]
